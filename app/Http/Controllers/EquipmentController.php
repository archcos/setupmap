...
class EquipmentController extends Controller
{
    ...
    public function __construct(SupabaseService $supabaseService)
    {
        ...
        Log::info('EquipmentController initialized');
    }
    ...
    {
        Log::info('Rendering equipment page');
    ...
    {
        Log::info('Fetching equipment index', [
        ...
        try {
            ...
            foreach ($equipments as $equipment) {
                try {
                    ...
                    Log::error('Failed to enrich equipment data', [
                        'equipment_id' => $equipment['equipment_id'] ?? 'unknown',
                        'error' => $e->getMessage(),
                    ]);
                    // Continue with next equipment instead of failing completely
                    continue;
                }
            }

            Log::info('Successfully fetched equipment index', [
                'total_equipments' => count($equipments),
                'enriched_count' => count($enrichedEquipments),
            ]);

            return response()->json($enrichedEquipments);
        } catch (\Exception $e) {
            Log::error('Failed to fetch equipment index', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return response()->json(['error' => 'Failed to fetch equipment data: '.$e->getMessage()], 500);
        }
    }
    ...
    {
        Log::info('Fetching specific equipment', [
    ...
    {
        Log::info('Rendering equipment details page', ['equipment_id' => $equipmentId]);

        try {
            $equipment = $this->supabaseService->getEquipmentById($equipmentId);

            if (!$equipment) {
                Log::warning('Equipment not found for details page', ['equipment_id' => $equipmentId]);
            ...
            Log::info('Successfully rendered equipment details page', ['equipment_id' => $equipmentId]);

            return Inertia::render('Equipment/Details', [
                'equipment' => $enrichedEquipment,
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to render equipment details page', [
                'equipment_id' => $equipmentId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    {
        Log::info('Fetching utilization data', [
            'start_date' => $request->query('start_date'),
            'end_date' => $request->query('end_date'),
        ]);
        ...
        try {
            ...
            foreach ($equipments as $equipment) {
                try {
                    ...
                    Log::error('Failed to enrich equipment for utilization data', [
                        'equipment_id' => $equipment['equipment_id'] ?? 'unknown',
                        'error' => $e->getMessage(),
                    ]);
                }
            ...
            Log::info('Successfully fetched utilization data', [
                'equipment_count' => count($utilizationData),
            ]);

            return response()->json($utilizationData);
        } catch (\Exception $e) {
            Log::error('Failed to fetch utilization data', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return response()->json(['error' => 'Failed to fetch utilization data: '.$e->getMessage()], 500);
        }
    }
    ...
    public function mapData(Request $request): JsonResponse
    {
        ...
        try {
            ...
            foreach ($equipments as $equipment) {
                try {
                    ...
                    if (!empty($locations)) {
                        ...
                        Log::warning('Equipment has no locations for map data', ['equipment_id' => $equipmentId]);
                    }
                } catch (\Exception $e) {
                    ++$errorCount;
                    Log::error('Failed to process equipment for map data', [
                        'equipment_id' => $equipment['equipment_id'] ?? 'unknown',
                        'error' => $e->getMessage(),
                    ]);
                }
            ...
            Log::info('Successfully fetched map data', [
                'total_equipments' => count($equipments),
                'map_data_count' => count($mapData),
                'error_count' => $errorCount,
            ]);

            return response()->json($mapData);
        } catch (\Exception $e) {
            Log::error('Failed to fetch map data', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return response()->json(['error' => 'Failed to fetch map data: '.$e->getMessage()], 500);
        }
    }
    ...
    {
        Log::info('Calculating weekly utilization history', ['equipment_id' => $equipmentId]);
        ...
        try {
            ...
            Log::info('Successfully calculated weekly utilization history', [
                'equipment_id' => $equipmentId,
                'days_calculated' => count($weeklyData),
            ]);

            return $weeklyData;
        } catch (\Exception $e) {
            Log::error('Failed to calculate weekly utilization history', [
                'equipment_id' => $equipmentId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    private function calculateUtilizationPercentage(array $utilizationRecords, ?string $startDate, ?string $endDate): float
    {
        ...
        if ($startDate && $endDate) {
            try {
                ...
                Log::error('Error calculating total hours for utilization percentage', [
                    'startDate' => $startDate,
                    'endDate' => $endDate,
                    'error' => $e->getMessage(),
                ]);
    ...
    private function filterRecordsByDateRange(array $records, ?string $startDate = null, ?string $endDate = null): array
    {
        ...
        try {
            ...
            Log::error('Error filtering records by date range', [
                'startDate' => $startDate,
                'endDate' => $endDate,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $equipmentId = $equipment['equipment_id'];
        Log::debug('Enriching equipment data', ['equipment_id' => $equipmentId]);
        ...
        try {
            ...
            Log::debug('Successfully enriched equipment data', ['equipment_id' => $equipmentId]);
